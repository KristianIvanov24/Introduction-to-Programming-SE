...
`num = 2` -> преминаваме към `case 2`<br>
`case 2: num *= 3;` -> num = 2 * 3 = 6<br>
Няма `break`, затова изпълнението продължава към `case 3`<br>
`case 3: num -= 1;` -> num = 6 - 1 = 5<br>
Извеждаме num -> 5<br>
...
`a = 7`<br>
`a -= a += 7` -> операторите `+=` и `-=` са дясноасоциативни, затова първо се изпълнява `a += 7` -> a = 7 + 7 = 14<br>
След това се изпълнява `a -= 14` -> a = 14 - 14 = 0<br>
Условието `if(a)` -> `if(0)` е false<br>
Изпълнява се `else` -> `a += 3` -> a = 0 + 3 = 3<br>
Извеждаме `a` -> 3<br>
...
Изразът в `if(a, b)` използва оператора запетая `,`<br>
Операторът `,` изпълнява първо `a`, след това `b`, и връща стойността на последния елемент, т.е. `b = 5`<br>
`if(b)` -> `if(5)` е true, затова влизаме в `if`-a<br>
Извеждаме a -> 0<br>
...
`if (a < 2)` -> 5 < 2 е false<br>
Тъй като `if` без `{}` засяга само първия ред след него (ако има скоби {} взима целия блок), тоест `std::cout << "I am ";` се пропуска<br>
Вторият ред `std::cout << "Pesho";` се изпълнява винаги<br>
Извеждаме -> Pesho<br>
